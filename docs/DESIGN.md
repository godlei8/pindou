...
当前子集 = GB2312 一级汉字 3755 字 + 界面/后端文案用字 + ASCII + 常用标点 = 3911 码位（含品牌名的「馨」）。
...
- **名字**：「馨豆」——馨取「温馨」，豆是拼豆。
  浏览器标签页写「馨豆 · 拼豆图纸」，顶栏和登录页标题写「馨豆」，副标题交代用途「拼豆图纸生成」。
  「馨」不在 GB2312 一级字里，已单独补进字体子集（3911 码位）；改名或新增文案前先查子集有没有这个字。
